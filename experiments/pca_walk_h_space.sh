...
root_outdir=outputs/pca_walk_h_space
mkdir -p $root_outdir

CONFIG=configs/config.yaml

use_directional_text_conditionals_h_space=True
...
do
    ...
    do
        ...
        #     --use_directional_text_conditionals_h_space $use_directional_text_conditionals_h_space --alphas "${alphas[@]}" --component_ind $component_ind\
        #     --init_image $init_image --init_image_strength $init_image_strength \
        #     --num_steps $num_steps \
        #     --make_video $make_video --output_dir ${root_outdir}/walk_videos_pca_component_${j}_seed_${seed}_init \
        #     --gpuid 2

        python -u main.py  --config ${CONFIG} --prompts "${prompt[@]}" --seeds $seed \
            --use_directional_text_conditionals_h_space $use_directional_text_conditionals_h_space --alphas "${alphas[@]}" --component_ind $component_ind\
            --num_steps $num_steps \
            --make_video $make_video --output_dir ${root_outdir}/walk_videos_pca_component_${j}_seed_${seed} \
            --gpuid 2
