...
root_outdir=outputs/normal_walk
mkdir -p $root_outdir
...
do
    ...
    #     --num_steps $num_steps \
    #     --make_video $make_video --output_dir ${root_outdir}/walk_videos_seed_${seed}_init_test\
    #     --gpuid 2

    python -u main.py  --config ${CONFIG} --prompts "${prompt[@]}" --seeds $seed \
        --num_steps $num_steps \
        --make_video $make_video --output_dir ${root_outdir}/walk_videos_seed_${seed}_test \
        --gpuid 2
